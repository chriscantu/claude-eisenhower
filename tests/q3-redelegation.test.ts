 ...
 *   2. /complete-task re-delegates the task; TASKS.md Owner becomes 'jordan' and the
 *      title becomes "Check in: jordan re: API contract review".
 *   3. /complete-task done later → lookup uses the NEW title → AppleScript returns
 ...
 *   - complete-task.md forwards `Reminder-id` to the dispatcher, so the original
...
describe("Q3 re-delegation regression (issue #36)", () => {
  test("Q3RD-001: push captures Reminder-id; complete uses it after title change", async () => {
    ...
    // ── 2. Simulate /complete-task re-delegation: the task's Check-in title in
