 ...
 *
 * Issue #31 — /execute must show the matched record and require explicit
...
const executeMd = fs.readFileSync(
  path.join(__dirname, "..", "commands", "execute.md"),
  "utf8"
);

describe("Issue #31: /execute requires confirmation before mutation", () => {
  test("execute.md mandates matched-record preview before mutating", () => {
  ...
  test("execute.md requires explicit confirmation step", () => {
  ...
  test("execute.md handles ambiguous matches with numbered pick", () => {
  ...
  test("execute.md surfaces no-undo warning to user", () => {
