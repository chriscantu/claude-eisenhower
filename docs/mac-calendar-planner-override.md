...
See `docs/adrs/calendar-performance-fix.md` for the full diagnosis. In short: AppleScript walks every event object to evaluate date filters (O(n) on total calendar size). EventKit uses database indexing and returns instantly regardless of calendar size.
