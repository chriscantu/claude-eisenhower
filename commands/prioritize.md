...
- If a user reports a task is blocked: do NOT create a Blocked state. Instead:
  - **External blocker on an Active task** (vendor, security review, legal sign-off, peer's deliverable): set `Awaiting: <blocker name>` and a required `Check-by:` date. Use `Note:` for context. The task stays in `## Active`. `/org awaiting` rolls these up by blocker. See `docs/specs/tasks-schema-spec.md` Decision 6.
