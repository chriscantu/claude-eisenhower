...
1. **Check for stakeholder graph**: Read `plugin_root` from `integrations/config/task-output-config.md` (required — run /setup if not configured), then read `{plugin_root}/integrations/config/stakeholders.yaml`.
