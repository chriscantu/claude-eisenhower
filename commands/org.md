...
You are running the /org command of the Engineering Task Flow.
...
No config check needed — /org reads TASKS.md and delegation memory only.
...
3. **Check-by dates disagree.** → `{alias} / "{title}": TASKS.md Check-by
   {date_a}, memory Check-by {date_b}. TASKS.md wins for /org purposes.`
...
If the user provided an argument (e.g., `/org auth-migration`, `/org alex`, or `/org awaiting`):
...
resolution below. Trailing tokens after `awaiting` (e.g., `/org awaiting foo`)
are ignored.
...
```
Run /org [project] for detail, or /org [alias] for a person view.
```
...
```
Run /org for full org view, or /org {alias} for delegate view.
```
...
```
Run /org for full org view, or /org {project} for project view.
```
